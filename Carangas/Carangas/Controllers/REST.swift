...
class REST {
    ...
    class func loadCars(onComplete: @escaping ([Car]) -> Void, onError: @escaping (CarError) -> Void) {
        ...
        AF.request(REST.basePath).responseJSON { af in
            ...
            if af.error == nil {
                            ...
                            // 2
                guard let response: URLRequest = af.request else {
                                onError(.noResponse)
                                return
                            }
                            
                if af.response?.statusCode == 200 {
                                
                                // servidor respondeu com sucesso :)
                                
                                // 3
                                // obter o valor de data
                                guard let data = af.data else {
                                    ...
                                    switch af.result {
                                        case .success(let value as [Car]):
                                                    onComplete(value) // SUCESSO
    ...
    private class func applyOperation(car: Car, operation: RESTOperation , onComplete: @escaping (Bool) -> Void ){
        ...
        AF.request(url, method: httpMethod, encoding: JSONEncoding.default)
